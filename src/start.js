...
import { getToken, getTokenUpdateTime } from './routes/token/token.controller.js'
...
const testing = async () => {
  await advertiserService.getAllActiveAdvertisers();
}

;(async () => {
  await loginService.apiAuthLogin().then(async () => {
    switch (action) {
      case 0:
        await testing()
        break
      ...
      case 6:
        ...
        break
      case 7:
        advertiserService.findAdvertiser('FTI').then((id) => { console.log(id) })
        break
      case 8:
        lineItemService.updateLineItem(16456951)
        break
      case 9:
        segmentPixelService.findSegment('FTI-FTI-IMPRESSION').then((res) => {console.log(res)})
        break;
      case 10:
        creativeFolderService.createFolder(2682205, 'TEST', [{id: 217238072}, {id: 142319005}]).then((res) => {
          console.log(res);
        })
        break;
    }
